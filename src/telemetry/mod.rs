...
pub use adversarial::{
    aggressive_mode, deception_mode, detect_evasion, honeypot_enabled, jittered_elapsed_ms,
    log_deception_audit, max_event_len, plausible_allow_reason, print_decoy_allow,
    sanitize_event_for_model, should_decoy_allow, silent_quarantine_reason, DeceptionMode,
    EvasionSignals,
};
pub use allowlist_config::{
    daemon_is_toolchain_build, daemon_is_trusted, exe_has_own_stack_prefix, own_stack_exe_prefixes,
    own_stack_script_cmdline_prefixes, python3_script_cmdline_allowed,
};
pub use config::{
...
pub use never_fast_trust::{
    guard_event_skips_fast_trust, matches_never_fast_trust, NEVER_FAST_TRUST,
};
