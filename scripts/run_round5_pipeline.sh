...
python3 training/generate_threats.py --count "$A_COUNT" --out data/bucket_a_threats.jsonl

echo "[2/6] Bucket D — stretch MITRE (lateral, ransomware, impair, webshell)"
python3 generate_stretch_threats.py --count "$D_COUNT" --out data/bucket_d_stretch.jsonl

echo "[3/6] Bucket B — legit scary (widened)"
python3 generate_false_positive_armor.py --count "$B_COUNT" --out data/bucket_b_scary_legit.jsonl

echo "[4/6] Bucket C — ambiguous pairs (widened)"
python3 generate_ambiguous_pairs.py --pairs "$C_PAIRS" --out data/bucket_c_ambiguous.jsonl
...
python3 coverage/zero_gate.py --coverage data/mitre_coverage_v5.json --matrix coverage/matrix.yaml
