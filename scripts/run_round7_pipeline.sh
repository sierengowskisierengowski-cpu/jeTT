...
python3 training/generators/generate_eval_reinforce.py --failures data/eval_failures_r5.jsonl \
  --out data/bucket_j_eval_reinforce_r7.jsonl --variants 4 --round round7
if [[ -f data/eval_failures_r6.jsonl ]]; then
  python3 training/generators/generate_eval_reinforce.py --failures data/eval_failures_r6.jsonl \
    --out data/bucket_j2_eval_reinforce_r7.jsonl --variants 3 --round round7
fi

echo "[3/6] Own stack (cowrie, GNI, bifrost, Steam, Govee, jeTT)"
python3 training/generators/generate_own_stack.py --count 450 --seed 71 --out data/bucket_g2_own_stack_r7.jsonl

echo "[4/6] Threat depth"
python3 training/generators/generate_threats.py --count 500 --out data/bucket_a2_threats_r7.jsonl
python3 training/generators/generate_stretch_threats.py --count 300 --out data/bucket_d2_stretch_r7.jsonl
...
python3 training/coverage/zero_gate.py --coverage data/mitre_coverage_v7.json
